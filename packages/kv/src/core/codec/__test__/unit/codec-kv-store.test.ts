...
describe("CodecKeyValueStore<T>", () => {
  let bytesStore: Mock<BytesKeyValueStore>
  let codec: Mock<Codec<string>>

  let codecStore: CodecKeyValueStore<string>

  beforeEach(() => {
    bytesStore = mock<BytesKeyValueStore>()
    codec = mock<Codec<string>>()

    codecStore = new CodecKeyValueStore<string>({
      bytesStore,
      codec,
    })
  })
  ...
  describe("get", () => {
    it("returns miss when underlying BytesKeyValueStore misses (does not decode)", async () => {
      const key = "k:miss"
    ...
    it("decodes bytes when underlying BytesKeyValueStore hits", async () => {
      const key = "k:hit"
  ...
  describe("set", () => {
    it("encodes value before writing to underlying BytesKeyValueStore", async () => {
      const key = "k:set"
      const value = "hello"
  ...
  describe("delete", () => {
    it("passes through to underlying BytesKeyValueStore", async () => {
      const key = "k:inv"
  ...
  describe("has", () => {
    it("returns true when underlying BytesKeyValueStore reports key exists", async () => {
    ...
    it("returns false when underlying BytesKeyValueStore reports key does not exist", async () => {
  ...
  describe("error propagation", () => {
    it("propagates codec.encode errors (does not swallow)", async () => {
      ...
      const promise = codecStore.set("k:1", "value")

      await expect(promise).rejects.toBe(err)
      expect(bytesStore.set).not.toHaveBeenCalled()
    })
    ...
    it("propagates codec.decode errors (does not swallow)", async () => {
      ...
      const promise = codecStore.get("k:1")

      await expect(promise).rejects.toBe(err)
    })

    it("propagates underlying BytesKeyValueStore errors (does not swallow)", async () => {
      const err = new Error("bytes failed")
      bytesStore.get.mockRejectedValue(err)
